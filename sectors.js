...
// =============================================================================
// STATE (shared key with screener.html via the same STORAGE_KEY)
// =============================================================================
...
// URL params can anchor the view to a historical month (set by sankey.html on click).
...
function setIdleStatus(fallback) {
    ...
    if (month) {
        const link = isAtLatest()
                     ? ''
                     : ` · <a href="sectors.html?window=${state.window}">Reset to now</a>`;
...
(async function boot() {
    ...
    // Look up cache for every sector; if all 11 are cached, render without network.
