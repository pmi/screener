...
// Shares cache & state keys with sectors.html so data is reused.
// =============================================================================
...
// =============================================================================
// HELPERS (copied from sectors.js for self-containment)
// =============================================================================
...
function renderChart() {
    ...
    for (let i = 0; i < sectorData.length; i++) {
        ...
        // (13px) to match screener.css `td`/`td.ticker` exactly.
    ...
    captureRect.addEventListener('click', (e) => {
        ...
        window.location.href = `sectors.html?month=${encodeURIComponent(ym)}&window=1`;
    });
...
async function refresh(force) {
    ...
    try {
        ...
        if (failed.length === SECTORS.length) {
            ...
            setStatus(`${failed.length} sector(s) failed: ${failed.map(s => s.etf).join(', ')}`, 'error');
        } else {
            setStatus('', null);
        }
        renderChart();
...
(async function boot() {
    ...
    // Try to render from cache without network if all 11 ETFs are fresh.
